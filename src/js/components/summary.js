...
export default class Summary extends Component {
  setSummary({ name }, cardList) {
    ...
    const tags = this._element.querySelector('.summary__tags');
    tags.innerHTML = '';
    tags.insertAdjacentHTML('beforeend', topKeywords.length > 0 ? `По ключевым словам: ${formatKeywords(topKeywords)}` : '');
  }
}
